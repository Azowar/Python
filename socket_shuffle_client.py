...
def Send(message, encrypted_char_mapping):
    ...
    client.send(message)
    client.send(encrypted_char_mapping)

def Receive():
    while not disconnected:
        try:
            server_decoded_message = client.recv(1024).decode()
            encrypted_server_mapping = client.recv(1024).decode()
            Decrypt_mapping(encrypted_server_mapping)
            Decode_message(server_decoded_message, decrypted_mapping)
